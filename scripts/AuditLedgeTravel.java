...
/** Original-map differential for ledge travel; the isolated oracle is never bundled. */
class AuditLedgeTravel {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException(
          "AuditLedgeTravel <PK3> <oracle> [queries/map] [map|all] [reach|finish]");
    boolean finish = args.length == 5 && args[4].equals("finish");
    if (args.length == 5 && !finish && !args[4].equals("reach"))
      throw new IllegalArgumentException("Unknown ledge travel mode");
      ...
      for (var entry :
        ...
        var movement =
            new LedgeReachMovement(
                ...
                Float.parseFloat(System.getProperty("craftq3.audit.gravity", "800")),
                Float.parseFloat(System.getProperty("craftq3.audit.maxVelocity", "320")));
        var reaches = map.reachabilities().stream().filter(r -> r.baseTravelType() == 7).toList();
        if (reaches.isEmpty()) {
          System.out.println(name + " has no ledges");
          continue;
        }
        ...
        for (int i = 0; i < count; i++) {
          ...
          commands
              ...
              .append(presence)
              .append('\n');
          commands.append(finish ? "finish " : "travel ").append(r.baseTravelType()).append(' ');
    ...
    System.out.printf(
        "Ledge travel maps=%d requests=%d exactDifferences=%d maxFloatError=%.9g seconds=%.3f%n",
        maps, total, mismatches, maxError, (System.nanoTime() - begin) / 1e9);
    if (mismatches != 0) throw new AssertionError("Ledge travel differences: " + mismatches);
  }
