"""Original qagame BSP admission with synthetic spawn points and unchanged original geometry."""
from pathlib import Path
import argparse,os,subprocess,tempfile,zipfile
root=Path(__file__).resolve().parent.parent
parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--classpath',required=True);args=parser.parse_args()
for profile in ['retail','modern']:
 out=root/'.tools/map-loadout'/profile;out.mkdir(parents=True,exist_ok=True)
 with tempfile.TemporaryDirectory(prefix='games-',dir=out) as temporary:
  ...
  command=[str(Path(os.environ['JAVA_HOME'])/'bin/java'),'-cp',args.classpath,str(root/'scripts/AuditMapLoadout.java'),str(games)]
