"""Original qagame/cgame item pickups on mutable host terrain."""
...
for profile in ['retail','modern']:
 out=root/'.tools/bridge-pickups'/profile;out.mkdir(parents=True,exist_ok=True)
 with tempfile.TemporaryDirectory(prefix='games-',dir=out) as temporary:
  ...
  command=[str(Path(os.environ['JAVA_HOME'])/'bin/java'),'-cp',args.classpath,str(root/'scripts/AuditBridgePickups.java'),str(games)]
