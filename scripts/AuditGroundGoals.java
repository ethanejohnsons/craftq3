...
/** Full dry-ground move-to-goal differential against an isolated native development oracle. */
class AuditGroundGoals {
  ...
  private record Native(String result, String state, String action) {}

  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException("AuditGroundGoals <PK3> <oracle> [queries/map] [map|all] [air|air-crouch|air-pad|air-ledge|teleport|air-teleport|barrier|air-barrier]");
    ...
    int requestedTravel = barrier ? 4 : teleport ? 10 : airbornePad ? 18 : airborneLedge ? 7 : 0;
    ...
    Map<String, Integer> reasons = new TreeMap<>(),
        ...
        diffScenarios = new TreeMap<>(), commandKinds = new TreeMap<>();
    ...
    try (var zip = new ZipFile(Path.of(args[0]).toFile())) {
      for (var entry :
        ...
        try (var states =
          ...
          var jumpPads = new JumpPadMovement(new MovementObstruction(world,
              a -> a <= 0 ? 0 : map.areaSettings().get(a).reachabilityCount()));
          var ledges = new LedgeReachMovement(new MovementObstruction(world,
              a -> a <= 0 ? 0 : map.areaSettings().get(a).reachabilityCount()));
          var teleports = new TeleportReachMovement(new MovementObstruction(world,
              a -> a <= 0 ? 0 : map.areaSettings().get(a).reachabilityCount()));
          var barriers = new BarrierReachMovement(new MovementObstruction(world,
              a -> a <= 0 ? 0 : map.areaSettings().get(a).reachabilityCount()));
          var move =
              new GroundMoveToGoal(
                  ...
                  e -> false,
                  ground,
                  Map.of(4, barriers::execute, 7, ledges::execute, 10, teleports::execute, 18, jumpPads::execute), Map.of(4, barriers::finish, 7, ledges::finish, 18, jumpPads::finish));
          ...
          if (candidates.isEmpty()) { System.out.println(name + " no eligible source areas"); continue; }
          ...
          for (int i = 0; i < count; i++) {
            ...
            int policyFlags = TravelFlags.DEFAULT;
            int first = 0;
            if (source > 0) {
              ...
              for (int r = set.firstReachability();
                  ...
                  r++)
                if (requestedTravel != 0 ? map.reachabilities().get(r).baseTravelType() == requestedTravel :
            ...
            if (barrier && !airborne && first != 0 && i % 3 == 0) {
              var start = map.reachabilities().get(first).start();
              origin = new Vec3((float) start.x() + i % 7 - 3,
                  (float) start.y() + i % 5 - 2, (float) start.z() + .125f);
            }
            ...
            if (airborne) {
              if (first == 0) { skipped++; mapSkip++; reasons.merge("No source WALK/CROUCH reach", 1, Integer::sum); continue; }
              var reach = map.reachabilities().get(first);
              origin = new Vec3((float) reach.start().x() + random.nextFloat() * 64 - 32,
                  (float) reach.start().y() + random.nextFloat() * 64 - 32,
                  (float) reach.start().z() + random.nextFloat() * 128 + .5f);
              if (predictor.onGround(origin, presence, 1)) { skipped++; mapSkip++; reasons.merge("AAS grounded sample", 1, Integer::sum); continue; }
              effective = mode % 3 == 0 ? 512 : mode % 3 == 1 ? 0 : 4 | 8 | 128 | 1;
              history = new BotMovement.History(mode % 3 == 0 ? 0 : source, source, source,
                  first, source, first, mode % 2 == 0 ? -5 : 99, new Vec3(7, 8, 9));
              world.floor = (float) origin.z() - 25;
            ...
            var input =
                new MovementInit(
                    origin,
                    airborneKind != 0 ? new Vec3(random.nextFloat() * 800 - 400, random.nextFloat() * 800 - 400, random.nextFloat() * 800 - (airborneLedge ? 600 : 0)) : ZERO,
                    new Vec3(0, 0, 26),
                    1,
                    1,
                    .1f,
            ...
            var seed = new BotInput(0,
                barrier ? new Vec3(.25f + i % 8 * .03125f, -.5f, .125f) : ZERO,
                barrier ? 123 : 0, ZERO,
                barrier ? switch (i % 4) { case 1 -> 1; case 2 -> ActionFlags.JUMPED_LAST_FRAME;
                  case 3 -> ActionFlags.JUMPED_LAST_FRAME | ActionFlags.JUMP; default -> 0; } : 0, 0);
            ...
            try {
              ...
              skipped++;
              mapSkip++;
              reasons.merge(unsupported instanceof GroundMoveToGoal.UnsupportedMovement detail ? detail.reason() : unsupported.getMessage(), 1, Integer::sum);
              continue;
            }
            ...
            if (airborne) scenario = "air-" + map.reachabilities().get(first).baseTravelType() + "-" + scenario;
            expected.add(
                new Expected(
                    caseCommands,
                    scenario,
            ...
            commandKinds.merge((output.writtenBytes() == 24 ? "prefix" :
                output.command().isPresent() ? "move" : "full-no-move") + ":" + output.actionFlags(),
                1, Integer::sum);
          }
    ...
    System.out.printf(
        "Ground goals maps=%d compared=%d skipped=%d differences=%d seconds=%.3f%n",
  ...
  private static boolean hasTravel(AasMap map, int area, int kind) {
    ...
    for (int i = setting.firstReachability(); i < setting.firstReachability() + setting.reachabilityCount(); i++)
      if (map.reachabilities().get(i).baseTravelType() == kind) return true;
    return false;
  }

  private static AasMap crouchMap(AasMap map) {
    var reaches = map.reachabilities().stream().map(r -> r.baseTravelType() != 2 ? r :
        new AasMap.Reachability(r.area(), r.face(), r.edge(), r.start(), r.end(), 3, r.travelTime(), r.reserved())).toList();
    return new AasMap(map.version(), map.bspChecksum(), map.lumps(), map.boundingBoxes(), map.vertices(),
        map.planes(), map.edges(), map.edgeIndices(), map.faces(), map.faceIndices(), map.areas(),
        map.areaSettings(), reaches, map.nodes(), map.portals(), map.portalIndices(), map.clusters());
  }

  private static String commands(
      MovementInit in, BotMovement.Snapshot state, Goal goal, int flags, World world, BotInput seed) {
  ...
  private static String compare(Expected e, Native n) {
    ...
    float[] values = {(float) actions.direction().x(), (float) actions.direction().y(),
        (float) actions.direction().z(), actions.speed()};
    for (int i = 0; i < 4; i++)
  ...
  private static BotInput applyActions(BotInput seed, GroundMoveToGoal.Output output) {
    try (var actions = new ElementaryActions(1, (client, text) -> {})) {
      ...
      actions.action(0, output.actionFlags() & ~ActionFlags.JUMP);
  ...
  private static List<Native> oracle(String exe, String pk3, String map, String commands)
      throws Exception {
    Process process =
        new ProcessBuilder(exe, pk3, map).redirectError(ProcessBuilder.Redirect.DISCARD).start();
