...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  ...
  if(!strncmp(line,"goal ",5)) {
   ...
   api->ea.EA_ResetInput(ms->client);if(fields==10){api->ea.EA_Move(ms->client,seed,seedSpeed);api->ea.EA_Action(ms->client,seedFlags);}api->ai.BotMoveToGoal(&result,mh,&g,flags);
