    ...
    try(var fs=Pk3FileSystem.mount(Path.of(args[0]),"baseq3");
      ...
      try(var game=new Q3Server(fs,"q3dm1",BspReader.read(fs.read(new VirtualPath("maps/q3dm1.bsp"))),null,System.out::print,Clock.systemUTC(),null,cvars)) {
        ...
        try(var host=new Protocol68Host(game,fs,12345,0,System.out::println);
          ...
          double[][] initial=new double[2][];double[] moved={0,0};int sent=0,dropped=0;
          for(int frame=0;frame<700;frame++) {
            ...
            for(int i=0;i<128;i++){
              ...
              if(!udp.send(value.peer(),value.payload()))break;host.packetSent();
            }
          }
          ...
          check(snapshots[0]>100&&snapshots[1]>100,"Snapshots missing: "+Arrays.toString(snapshots));
          ...
          System.out.println("PASS hosted UDP "+game.abi()+" pure clients2 snapshots"+Arrays.toString(snapshots)+" moved"+Arrays.toString(moved)+" dropped"+dropped+" disconnect");
        }
      }
    }
  }
  static Map<String,String> info(String name){return Map.of("name",name,"model","sarge/default","rate","25000","snaps","20");}
