...
extern bot_moveresult_t BotTravel_RocketJump(bot_movestate_t*,aas_reachability_t*);
extern bot_moveresult_t BotTravel_BFGJump(bot_movestate_t*,aas_reachability_t*);
extern bot_moveresult_t BotFinishTravel_WeaponJump(bot_movestate_t*,aas_reachability_t*);
#include <archive.h>
...
static int configuredLastReach,configuredJumpReach;
...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  traceCount=0;
  if(sscanf(line,"reachdata %d",&area)==1){if(area<0||area>=aasworld.reachabilitysize)abort();aas_reachability_t*r=&aasworld.reachability[area];printf("REACHDATA %d %d %.9g %.9g %.9g %.9g %.9g %.9g\n",area,r->traveltype,r->start[0],r->start[1],r->start[2],r->end[0],r->end[1],r->end[2]);}
  else if(sscanf(line,"reachstate %d %d",&configuredLastReach,&configuredJumpReach)==2){}
  else if(sscanf(line,"random %d",&randomValue)==1) { if(randomValue<0||randomValue>32767)abort(); }
  ...
  else if(!strncmp(line,"travel ",7)||!strncmp(line,"finish ",7)) {
   bot_movestate_t ms={0};aas_reachability_t reach={0};ms.presencetype=movePresence;ms.entitynum=0;ms.client=0;ms.lastreachnum=configuredLastReach;ms.jumpreach=configuredJumpReach;
   ...
   if(reach.traveltype!=12&&reach.traveltype!=13)abort();
   ...
   EA_ResetInput(0);EA_ResetInput(0);if(seedMove)EA_Move(0,seedDirection,seedSpeed);EA_Action(0,seedActions);EA_SelectWeapon(0,7);vec3_t priorView={1,2,3};EA_View(0,priorView);randomDraws=0;
   bot_moveresult_t result=!strncmp(line,"finish ",7)?BotFinishTravel_WeaponJump(&ms,&reach):reach.traveltype==12?BotTravel_RocketJump(&ms,&reach):BotTravel_BFGJump(&ms,&reach);
   ...
   printf("REACHAFT %.9g %.9g %.9g %.9g %.9g %.9g\n",reach.start[0],reach.start[1],reach.start[2],reach.end[0],reach.end[1],reach.end[2]);printf("DRAWS %d\n",randomDraws);printf("EAEX %d %.9g %.9g %.9g\n",in.weapon,in.viewangles[0],in.viewangles[1],in.viewangles[2]);
  }
