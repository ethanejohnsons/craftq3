/* Authored elevator development host, forked from the authored platform observer. Links unchanged upstream native objects; no engine routines reproduced. */
...
extern void BotFuncBobStartEnd(aas_reachability_t*,vec3_t,vec3_t,vec3_t);
extern int BotOnMover(vec3_t,int,aas_reachability_t*);
extern bot_moveresult_t BotTravel_Elevator(bot_movestate_t*,aas_reachability_t*);
extern bot_moveresult_t BotFinishTravel_Elevator(bot_movestate_t*,aas_reachability_t*);
extern bot_moveresult_t BotTravel_FuncBobbing(bot_movestate_t*,aas_reachability_t*);
extern bot_moveresult_t BotFinishTravel_FuncBobbing(bot_movestate_t*,aas_reachability_t*);
#include <archive.h>
...
static int selectedReach,lastReach,jumpReach;static float deadlineSeed=77;
static int metadataOverride,faceOverride,edgeOverride;
static int boxOverride;static vec3_t boxLow,boxHigh;
static int stateSeed=0;static int moveEntity=0;static uint32_t queryHash=2166136261u;static int entityCallback;
static void hashWord(uint32_t v){queryHash=(queryHash^v)*16777619u;}
static void hashVec(vec3_t v){for(int k=0;k<3;k++){uint32_t bits;memcpy(&bits,&v[k],4);hashWord(bits);}}
...
static int replyUsed[16],replyEntity[16],replySolid[16];static float replyFraction[16];
...
static void modelBounds(int model,vec3_t angles,vec3_t mins,vec3_t maxs,vec3_t origin) {
 vec3_t zeroAngles={0},localMins,localMaxs,localOrigin;
 if(!angles)angles=zeroAngles;if(!mins)mins=localMins;if(!maxs)maxs=localMaxs;if(!origin)origin=localOrigin;
 hashWord(2);hashWord(model);hashVec(angles);
 if(traceVerbose)fprintf(stderr,"MODEL_BOUNDS %d angles %.9g %.9g %.9g\n",model,angles[0],angles[1],angles[2]);
 if(boxOverride) {VectorCopy(boxLow,mins);VectorCopy(boxHigh,maxs);VectorClear(origin);return;}
...
static void trace(bsp_trace_t *t,vec3_t start,vec3_t mins,vec3_t maxs,vec3_t end,int pass,int mask) {
 hashWord(entityCallback);hashVec(start);hashVec(mins);hashVec(maxs);hashVec(end);hashWord(pass);hashWord(mask);
 ...
 if(collisionMode==4 && traceCount<=16 && replyUsed[traceCount-1]) {int i=(int)traceCount-1;t->fraction=replyFraction[i];t->ent=replyEntity[i];t->startsolid=(replySolid[i]&1)!=0;t->allsolid=(replySolid[i]&2)!=0;for(int j=0;j<3;j++)t->endpos[j]=start[j]+t->fraction*(end[j]-start[j]);t->plane.normal[2]=1;t->contents=1;return;}
...
static void entityTrace(bsp_trace_t *t,vec3_t s,vec3_t lo,vec3_t hi,vec3_t e,int entity,int mask){if(traceVerbose)fprintf(stderr,"ENTITYTRACE target %d\n",entity);entityCallback=1;trace(t,s,lo,hi,e,entity,mask);entityCallback=0;}
...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  ...
  if(sscanf(line,"random %d",&randomValue)==1) { if(randomValue<0||randomValue>32767)abort(); }
  ...
  else if(sscanf(line,"listtype %d",&area)==1) { for(int n=1;n<aasworld.reachabilitysize;n++){aas_reachability_t reach={0};AAS_ReachabilityFromNum(n,&reach);if((reach.traveltype&0xffffff)==area)printf("RECORD %d %d %d %d %d %.9g %.9g %.9g %.9g %.9g %.9g\n",n,reach.traveltype,reach.areanum,reach.facenum,reach.edgenum,reach.start[0],reach.start[1],reach.start[2],reach.end[0],reach.end[1],reach.end[2]);} }
  else if(!strncmp(line,"bobfixture ",11)) { aas_reachability_t reach={0};vec3_t start={0},end={0},origin={0};if(sscanf(line+11,"%d %d",&reach.facenum,&reach.edgenum)!=2)abort();reach.traveltype=19;BotFuncBobStartEnd(&reach,start,end,origin);printf("BOBFIXTURE %d %d %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g\n",reach.facenum,reach.edgenum,start[0],start[1],start[2],end[0],end[1],end[2],origin[0],origin[1],origin[2]); }
  else if(sscanf(line,"bobmeta %d",&area)==1) { aas_reachability_t reach={0};vec3_t start={0},end={0},origin={0};AAS_ReachabilityFromNum(area,&reach);BotFuncBobStartEnd(&reach,start,end,origin);printf("BOBMETA %d face=%d edge=%d start=%.9g,%.9g,%.9g end=%.9g,%.9g,%.9g origin=%.9g,%.9g,%.9g\n",area,reach.facenum,reach.edgenum,start[0],start[1],start[2],end[0],end[1],end[2],origin[0],origin[1],origin[2]); }
  else if(!strncmp(line,"onmover ",8)) { vec3_t origin;int entity,num;if(sscanf(line+8,"%d %d %f %f %f",&num,&entity,&origin[0],&origin[1],&origin[2])!=5)abort();aas_reachability_t reach={0};AAS_ReachabilityFromNum(num,&reach);printf("ONMOVER %d\n",BotOnMover(origin,entity,&reach)); }
  else if(sscanf(line,"box %f %f %f %f %f %f",&boxLow[0],&boxLow[1],&boxLow[2],&boxHigh[0],&boxHigh[1],&boxHigh[2])==6)boxOverride=1;
  else if(!strncmp(line,"entity ",7)) { int number;bot_entitystate_t state={0};if(sscanf(line+7,"%d %d %f %f %f",&number,&state.modelindex,&state.origin[0],&state.origin[1],&state.origin[2])!=5)abort();state.type=4;state.solid=3;VectorCopy(state.origin,state.old_origin);api->BotLibUpdateEntity(number,&state); }
  else if(sscanf(line,"reachmeta %d %d",&faceOverride,&edgeOverride)==2)metadataOverride=1;
  else if(sscanf(line,"unentity %d",&area)==1){api->BotLibUpdateEntity(area,NULL);}
  else if(!strncmp(line,"frame ",6)){float time;if(sscanf(line+6,"%f",&time)!=1)abort();api->BotLibStartFrame(time);}
  else if(sscanf(line,"originmodel %d",&area)==1){vec3_t origin={123,456,789};int result=AAS_OriginOfMoverWithModelNum(area,origin);printf("ORIGINMODEL %d %.9g %.9g %.9g\n",result,origin[0],origin[1],origin[2]);}
  else if(!strncmp(line,"entityfull ",11)){int number;bot_entitystate_t state={0};if(sscanf(line+11,"%d %d %f %f %f %d %d",&number,&state.modelindex,&state.origin[0],&state.origin[1],&state.origin[2],&state.type,&state.solid)!=7)abort();VectorCopy(state.origin,state.old_origin);api->BotLibUpdateEntity(number,&state);}
  else if(sscanf(line,"selected %d",&selectedReach)==1){}
  else if(sscanf(line,"history %d %d",&lastReach,&jumpReach)==2){}
  else if(sscanf(line,"deadline %f",&deadlineSeed)==1){}
   ...
   bot_movestate_t ms={0};aas_reachability_t reach={0};ms.presencetype=movePresence;ms.entitynum=moveEntity;ms.client=0;
   ...
   if(reach.traveltype!=11&&reach.traveltype!=19)abort();
   if(selectedReach) { aas_reachability_t original={0};AAS_ReachabilityFromNum(selectedReach,&original);reach.facenum=original.facenum;reach.edgenum=original.edgenum;reach.areanum=original.areanum;fprintf(stderr,"REACH %d face=%d edge=%d area=%d type=%d start=%.9g,%.9g,%.9g end=%.9g,%.9g,%.9g\n",selectedReach,original.facenum,original.edgenum,original.areanum,original.traveltype,original.start[0],original.start[1],original.start[2],original.end[0],original.end[1],original.end[2]); }
   if(metadataOverride){reach.facenum=faceOverride;reach.edgenum=edgeOverride;}
   if(stateSeed){ms.lastareanum=13;ms.lastgoalareanum=31;ms.reachareanum=23;VectorSet(ms.lastorigin,1,2,3);ms.grapplevisible_time=37;ms.lastgrappledist=41;ms.avoidreach[0]=17;ms.avoidreachtimes[0]=91;ms.avoidreachtries[0]=4;ms.numavoidspots=1;VectorSet(ms.avoidspots[0].origin,11,12,13);ms.avoidspots[0].radius=7;ms.avoidspots[0].type=2;}
   ms.lastreachnum=lastReach;ms.jumpreach=jumpReach;ms.reachability_time=deadlineSeed;
   ...
   EA_ResetInput(0);EA_ResetInput(0);vec3_t seedView={1,2,3};EA_View(0,seedView);EA_SelectWeapon(0,7);if(seedMove)EA_Move(0,seedDirection,seedSpeed);EA_Action(0,seedActions);randomDraws=0;
   if(traceVerbose)fprintf(stderr,"CALL\n");queryHash=2166136261u;bot_movestate_t before=ms;
   bot_moveresult_t result=!strncmp(line,"finish ",7)?(reach.traveltype==11?BotFinishTravel_Elevator(&ms,&reach):BotFinishTravel_FuncBobbing(&ms,&reach)):(reach.traveltype==11?BotTravel_Elevator(&ms,&reach):BotTravel_FuncBobbing(&ms,&reach));
   ...
   printf("QHASH %u\n",queryHash);printf("DEADLINE %.9g\n",ms.reachability_time);printf("MUTATIONS");for(int k=0;k<sizeof(ms);k+=4){uint32_t a,b;memcpy(&a,(byte*)&before+k,4);memcpy(&b,(byte*)&ms+k,4);if(a!=b)printf(" %d:%08x>%08x",k,a,b);}putchar('\n');printf("DRAWS %d\n",randomDraws);printf("EFFECTS %.9g %.9g %.9g %d\n",in.viewangles[0],in.viewangles[1],in.viewangles[2],in.weapon);
  }
  else if(sscanf(line,"stateseed %d",&stateSeed)==1){}
  else if(sscanf(line,"moveentity %d",&moveEntity)==1){}
  else if(!strncmp(line,"repliesclear",12)){memset(replyUsed,0,sizeof(replyUsed));collisionMode=4;}
  else if(!strncmp(line,"reply ",6)){int n,e,s;float f;if(sscanf(line+6,"%d %f %d %d",&n,&f,&e,&s)!=4||n<1||n>16)abort();replyUsed[n-1]=1;replyFraction[n-1]=f;replyEntity[n-1]=e;replySolid[n-1]=s;collisionMode=4;}
