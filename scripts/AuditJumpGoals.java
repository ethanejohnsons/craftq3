...
/** Full jump move-to-goal differential against an isolated native development oracle. */
class AuditJumpGoals {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException(
          "AuditJumpGoals <PK3> <oracle> [queries/map] [map|all]" + " [jump|air-jump|ground]");
    ...
    boolean jump = args.length == 5 && args[4].endsWith("jump") && !args[4].endsWith("waterjump");
    ...
    int requestedTravel =
        jump
            ...
            : waterJump
                ? 9
                : swim ? 8 : barrier ? 4 : teleport ? 10 : airbornePad ? 18 : airborneLedge ? 7 : 0;
    ...
    try (var zip = new ZipFile(Path.of(args[0]).toFile())) {
      for (var entry :
        ...
        try (var states =
          ...
          var jumps = new JumpReachMovement(runStart::calculate, nav::pointArea);
          var move =
              new GroundMoveToGoal(
                  ...
                  Map.of(5, jumps::execute),
                  Map.of(5, jumps::finish));
    ...
    System.out.printf(
        "Jump goals maps=%d compared=%d skipped=%d differences=%d seconds=%.3f%n",
