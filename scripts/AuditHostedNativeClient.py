"""Both original gameplay ABIs with native established-client packet operations; loopback only."""
...
for profile in (['retail','modern'] if args.profile=='both' else [args.profile]):
 out=root/'.tools/hosted-native-client-oracle'/profile;out.mkdir(parents=True,exist_ok=True)
 with tempfile.TemporaryDirectory(prefix='games-',dir=out) as temporary:
  ...
  command=[str(Path(os.environ['JAVA_HOME'])/'bin/java'),'-cp',args.classpath,str(root/'scripts/AuditHostedNativeClient.java'),str(games),str(root/'.tools/hosted-native-client-oracle/probe')]
  with (out/'audit.log').open('w') as log:result=subprocess.run(command,cwd=root,stdout=log,stderr=subprocess.STDOUT,timeout=90)
  if result.returncode:raise RuntimeError('Native packet audit failed: '+str(out/'audit.log'))
