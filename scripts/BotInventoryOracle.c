/* Authored inventory compatibility driver; unchanged native exports are QA-only.
 * Public declaration/layout metadata and call observations only; no native routines reproduced.
 * Optional argv[3] overrides only botfiles/inv.h reads in this process. */
...
static const char *inventoryHeader;
...
static int fsopen(const char *path,fileHandle_t *handle,fsMode_t mode) {
 ...
 FILE *file=NULL;
 if(inventoryHeader&&!strcmp(path,"botfiles/inv.h"))file=fopen(inventoryHeader,"rb");
...
int main(int argc,char **argv) {
 if(argc!=3&&argc!=4)return 2;root=argv[1];if(argc==4)inventoryHeader=argv[3];configfile=argv[2];botimport.Print=print;botimport.FS_FOpenFile=fsopen;botimport.FS_Read=fsread;botimport.FS_FCloseFile=fsclose;
