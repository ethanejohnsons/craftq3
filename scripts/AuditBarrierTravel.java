...
/** Original-map differential for barrier travel; the isolated oracle is never bundled. */
class AuditBarrierTravel {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException(
          "AuditBarrierTravel <PK3> <oracle> [queries/map] [map|all] [entry|finish]");
    boolean finishing = args.length == 5 && args[4].equals("finish");
    if (args.length == 5 && !finishing && !args[4].equals("entry"))
      throw new IllegalArgumentException("Unknown barrier travel mode");
    ...
    try (var zip = new ZipFile(Path.of(args[0]).toFile())) {
      for (var entry :
        ...
        var reaches = map.reachabilities().stream().filter(r -> r.baseTravelType() == 4).toList();
        if (reaches.isEmpty()) {
          System.out.println(name + " has no barrier reaches");
          continue;
        }
        var commands = new StringBuilder("clear\neaseed .25 -.5 .125 123\n");
        var expected = new ArrayList<BarrierReachMovement.Output>();
        ...
        for (int i = 0; i < count; i++) {
          ...
          var r = e.result();
          var move =
              e.movement().orElse(new BarrierReachMovement.Move(new Vec3(.25, -.5, .125), 123));
          int[] ints = {
            r.failure(),
            r.type(),
            r.blocked(),
    ...
    System.out.printf(
        "Barrier travel maps=%d requests=%d exactDifferences=%d maxFloatError=%.9g seconds=%.3f%n",
        maps, total, mismatches, maxError, (System.nanoTime() - begin) / 1e9);
    if (maps == 0) throw new IllegalArgumentException("No maps with barrier reaches selected");
    if (mismatches != 0) throw new AssertionError("Barrier travel differences: " + mismatches);
  }
