...
/** Full liquid move-to-goal differential against an isolated native development oracle. */
class AuditLiquidGoals {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException(
          "AuditLiquidGoals <PK3> <oracle> [queries/map] [map|all]"
              + " [swim|waterjump|air-swim|air-waterjump|liquid|air]");
        ...
        try (var states =
          ...
          for (int i = 0; i < count; i++) {
            ...
            var seed =
                new BotInput(
                    0,
                    (barrier || liquid || swim || waterJump)
                        ? new Vec3(.25f + i % 8 * .03125f, -.5f, .125f)
                        : ZERO,
                    (barrier || liquid || swim || waterJump) ? 123 : 0,
                    ZERO,
                    barrier
                        ? switch (i % 4) {
                          case 1 -> 1;
    ...
    System.out.printf(
        "Liquid goals maps=%d compared=%d skipped=%d differences=%d seconds=%.3f%n",
