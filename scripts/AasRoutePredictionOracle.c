/* Authored development host adapted from the team's AAS observer scaffolding.
 * Links unchanged native objects; no engine routine bodies copied or translated.
 * Public ABI metadata: ioquake/ioq3 code/botlib/{be_aas.h,be_aas_route.h,be_aas_def.h}.
 * This observer and its caller-owned original assets are never included in the mod.
 */
...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  int area,goal,flags,ma,mt,se,sc,st,sa;vec3_t p;
  if(sscanf(line,"predict %d %f %f %f %d %d %d %d %d %d %d %d",&area,&p[0],&p[1],&p[2],&goal,&flags,&ma,&mt,&se,&sc,&st,&sa)==12) {
   aas_predictroute_t r;memset(&r,0xa5,sizeof(r));
   int ok=AAS_PredictRoute(&r,area,p,goal,flags,ma,mt,se,sc,st,sa);
   printf("PREDICT %d %.9g %.9g %.9g %d %d %d %d %d %d\n",ok,r.endpos[0],r.endpos[1],r.endpos[2],r.endarea,r.stopevent,r.endcontents,r.endtravelflags,r.numareas,r.time);
  } else if(sscanf(line,"first %d %f %f %f %d %d",&area,&p[0],&p[1],&p[2],&goal,&flags)==6) {
   int time=0,reach=0;int ok=AAS_AreaRouteToGoalArea(area,p,goal,flags,&time,&reach);
   printf("FIRST %d %d %d\n",ok,time,reach);
  } else if(sscanf(line,"reachareas %d",&area)==1) {
   if(area<0||area>=aasworld.reachabilitysize)abort();
   aas_reachabilityareas_t *a=&aasworld.reachabilityareas[area];
   printf("REACHAREAS %d",a->numareas);
   for(int i=0;i<a->numareas;i++)printf(" %d",aasworld.reachabilityareaindex[a->firstarea+i]);puts("");
  } else if(!strncmp(line,"layout",6)) {
   printf("LAYOUT %zu %zu %zu %zu %zu %zu %zu %zu\n",sizeof(aas_predictroute_t),offsetof(aas_predictroute_t,endpos),offsetof(aas_predictroute_t,endarea),offsetof(aas_predictroute_t,stopevent),offsetof(aas_predictroute_t,endcontents),offsetof(aas_predictroute_t,endtravelflags),offsetof(aas_predictroute_t,numareas),offsetof(aas_predictroute_t,time));
