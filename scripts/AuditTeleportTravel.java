...
/** Original-map differential for teleport travel; the isolated oracle is never bundled. */
class AuditTeleportTravel {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException(
          "AuditTeleportTravel <PK3> <oracle> [queries/map] [map|all] [dry|swim]");
    boolean swimming = args.length == 5 && args[4].equals("swim");
    if (args.length == 5 && !swimming && !args[4].equals("dry"))
      throw new IllegalArgumentException("Unknown teleport travel mode");
    ...
    try (var zip = new ZipFile(Path.of(args[0]).toFile())) {
      for (var entry :
        ...
        var reaches = map.reachabilities().stream().filter(r -> r.baseTravelType() == 10).toList();
        if (reaches.isEmpty()) {
          System.out.println(name + " has no teleport reaches");
          continue;
        }
        ...
        for (int i = 0; i < count; i++) {
          ...
          int presence = i % 4 == 0 ? 4 : 2, flags = (i % 3 == 0 ? 514 : 2) | (swimming ? 4 : 0);
          var input =
              new MovementInit(
                  origin,
                  new Vec3(
                  ...
                  0);
          traceCount = 0;
          expected.add(movement.execute(input, flags, nav.pointArea(input.origin()), r));
          ...
          commands
              ...
              .append(presence)
              .append('\n');
          commands.append("travel ").append(r.baseTravelType()).append(' ');
    ...
    System.out.printf(
        "Teleport travel maps=%d requests=%d exactDifferences=%d maxFloatError=%.9g seconds=%.3f%n",
        maps, total, mismatches, maxError, (System.nanoTime() - begin) / 1e9);
    if (mismatches != 0) throw new AssertionError("Teleport travel differences: " + mismatches);
  }
