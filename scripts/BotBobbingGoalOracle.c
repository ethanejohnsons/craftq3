...
static int entityCallback,groundReply,groundEntity,groundSolid,platformReply,platformEntity,platformSolid;static float groundFraction,platformFraction;
...
static void trace(bsp_trace_t *t,vec3_t start,vec3_t mins,vec3_t maxs,vec3_t end,int pass,int mask) {
 ...
 if(platformReply&&!entityCallback&&mask==65537&&mins[0]==-16&&mins[1]==-16&&mins[2]==-8&&maxs[2]==8){t->fraction=platformFraction;t->ent=platformEntity;t->startsolid=(platformSolid&1)!=0;t->allsolid=(platformSolid&2)!=0;for(int j=0;j<3;j++)t->endpos[j]=start[j]+t->fraction*(end[j]-start[j]);t->plane.normal[2]=1;t->contents=1;return;}
 if(groundReply&&!entityCallback&&mask==65537&&end[0]==start[0]&&end[1]==start[1]&&end[2]==start[2]-3){t->fraction=groundFraction;t->ent=groundEntity;t->startsolid=(groundSolid&1)!=0;t->allsolid=(groundSolid&2)!=0;for(int j=0;j<3;j++)t->endpos[j]=start[j]+t->fraction*(end[j]-start[j]);t->plane.normal[2]=1;t->contents=1;return;}
...
static void entityTrace(bsp_trace_t *t,vec3_t s,vec3_t lo,vec3_t hi,vec3_t e,int entity,int mask){entityCallback=1;trace(t,s,lo,hi,e,entity,mask);entityCallback=0;}
...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  traceCount=0;
  if(sscanf(line,"platformhit %d %f %d",&platformEntity,&platformFraction,&platformSolid)==3){platformReply=1;continue;}
  if(!strncmp(line,"platformclear",13)){platformReply=0;continue;}
  if(sscanf(line,"entityinfo %d",&area)==1){aas_entityinfo_t e={0};api->aas.AAS_EntityInfo(area,&e);printf("ENTITYINFO %d type%d solid%d model%d origin%.9g,%.9g,%.9g mins%.9g,%.9g,%.9g maxs%.9g,%.9g,%.9g AREAS",area,e.type,e.solid,e.modelindex,e.origin[0],e.origin[1],e.origin[2],e.mins[0],e.mins[1],e.mins[2],e.maxs[0],e.maxs[1],e.maxs[2]);int count=0;for(aas_link_t *p=aasworld.entities[area].areas;p;p=p->next_area){if(++count>65536)abort();printf(" %d",p->areanum);}puts("");continue;}
  if(sscanf(line,"groundhit %d %f %d",&groundEntity,&groundFraction,&groundSolid)==3){groundReply=1;continue;}
  if(!strncmp(line,"groundclear",11)){groundReply=0;continue;}
  if(sscanf(line,"box %f %f %f %f %f %f",&boxLow[0],&boxLow[1],&boxLow[2],&boxHigh[0],&boxHigh[1],&boxHigh[2])==6){boxOverride=1;continue;}
  if(!strncmp(line,"repliesclear",12)){memset(replyUsed,0,sizeof(replyUsed));collisionMode=4;continue;}
  if(!strncmp(line,"reply ",6)){int n,e,s;float f;if(sscanf(line+6,"%d %f %d %d",&n,&f,&e,&s)!=4||n<1||n>16)abort();replyUsed[n-1]=1;replyFraction[n-1]=f;replyEntity[n-1]=e;replySolid[n-1]=s;collisionMode=4;continue;}
  if(!strncmp(line,"mover ",6)){int n;bot_entitystate_t e={0};if(sscanf(line+6,"%d %d %f %f %f %d %d",&n,&e.modelindex,&e.origin[0],&e.origin[1],&e.origin[2],&e.type,&e.solid)!=7)abort();VectorCopy(e.origin,e.old_origin);api->BotLibUpdateEntity(n,&e);continue;}
  if(sscanf(line,"tag %d",&area)==1){printf("TAG %d\n",area);fprintf(stderr,"TAG %d\n",area);continue;}
  if(sscanf(line,"listtype %d",&area)==1){for(int n=1;n<aasworld.reachabilitysize;n++){aas_reachability_t r;AAS_ReachabilityFromNum(n,&r);if((r.traveltype&0xffffff)==area)printf("RECORD %d %d %d %d %.9g %.9g %.9g %.9g %.9g %.9g\n",n,r.areanum,r.facenum,r.edgenum,r.start[0],r.start[1],r.start[2],r.end[0],r.end[1],r.end[2]);}continue;}
  ...
  if(!strncmp(line,"goal ",5)) {
   bot_goal_t g={0};int flags,seedFlags=0;vec3_t seed={0};float seedSpeed=0;bot_moveresult_t result;memset(&result,0x7f,sizeof(result));
   int fields=sscanf(line,"goal %f %f %f %d %d %f %f %f %f %d",&g.origin[0],&g.origin[1],&g.origin[2],&g.areanum,&flags,&seed[0],&seed[1],&seed[2],&seedSpeed,&seedFlags);
   if(fields!=5&&fields!=10)abort();
   VectorSet(g.mins,-15,-15,-15);VectorSet(g.maxs,15,15,15);g.entitynum=149;g.number=9;g.flags=1;g.iteminfo=11;
   api->ea.EA_ResetInput(ms->client);if(fields==10){api->ea.EA_Move(ms->client,seed,seedSpeed);api->ea.EA_Action(ms->client,seedFlags);}bot_input_t before;api->ea.EA_GetInput(ms->client,ms->thinktime,&before);if(traceVerbose)fprintf(stderr,"EA_BEFORE client%d %.9g %.9g %.9g %.9g %d\n",ms->client,before.dir[0],before.dir[1],before.dir[2],before.speed,before.actionflags);api->ai.BotMoveToGoal(&result,mh,&g,flags);
   ...
   bot_input_t input;api->ea.EA_GetInput(ms->client,ms->thinktime,&input);printf("EA %.9g %.9g %.9g %.9g %d VIEW %.9g %.9g %.9g WEAPON %d\n",input.dir[0],input.dir[1],input.dir[2],input.speed,input.actionflags,input.viewangles[0],input.viewangles[1],input.viewangles[2],input.weapon);
