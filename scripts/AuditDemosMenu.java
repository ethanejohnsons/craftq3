...
/**
 * CPU-only original Demos-menu file-list and command capture; no network, GUI or guest-memory
 * mutation.
 */
class AuditDemosMenu {
  public static void main(String[] args) throws Exception {
    if (args.length < 1)
      throw new IllegalArgumentException(
          "Usage: AuditDemosMenu games [ui.qvm|-] [keys comma-separated]");
    try (var disk = Pk3FileSystem.mount(Path.of(args[0]), "baseq3")) {
      var fs =
          new VirtualFileSystem() {
            ...
            public List<VirtualPath> list(String p) {
              return p.equals("demos")
                  ? List.of(
                      new VirtualPath("demos/legacy.dm3"),
                      new VirtualPath("demos/proto66.dm_66"),
                      new VirtualPath("demos/proto67.dm_67"),
                      new VirtualPath("demos/proto68.dm_68"),
                      new VirtualPath("demos/proto71.dm_71"))
                  : disk.list(p);
            }
      ...
      cvars.register("protocol", System.getProperty("audit.protocol", "68"), CvarSystem.ROM);
      ...
      try (var ui =
          new Q3Ui(
              ...
              System.out::print,
              host)) {
        var vmField = Q3Ui.class.getDeclaredField("vm");
        vmField.setAccessible(true);
        var vm = (dev.bluevista.craftq3.vm.QvmInterpreter) vmField.get(ui);
        var callbackField =
            dev.bluevista.craftq3.vm.QvmInterpreter.class.getDeclaredField("syscalls");
        callbackField.setAccessible(true);
        var original = (dev.bluevista.craftq3.vm.QvmSyscalls) callbackField.get(vm);
        callbackField.set(
            vm,
            (dev.bluevista.craftq3.vm.QvmSyscalls)
                (memory, call, arguments) -> {
                  if (call == 17)
                    System.out.println(
                        "FILELIST directory="
                            + memory.readCString(arguments[0], 8192)
                            + " extension="
                            + memory.readCString(arguments[1], 8192)
                            + " capacity="
                            + arguments[3]);
                  return original.invoke(memory, call, arguments);
                });
  ...
  private static void dump(CgameFrame frame) {
    for (var cmd : frame.commands())
      if (cmd instanceof CgameFrame.Quad q && (q.shader().startsWith("menu/art/")))
