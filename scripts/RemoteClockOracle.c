/* Authored CPU clock observer. Native clock routines are linked unchanged.
...
void CL_ReadDemoMessage(void) {demoCalls++;clc.state=CA_DISCONNECTED;}
...
int main(void) {
 ...
 while(scanf("%31s",command)==1){
  ...
  if(!strcmp(command,"reset")){
   ...
   paused=0;queue[0]=0;clc.state=CA_PRIMED;
