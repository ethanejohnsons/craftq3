...
source=root/'.tools/ioquake3-source/code';out=root/'.tools/pure-server-oracle';out.mkdir(parents=True,exist_ok=True)
...
subprocess.run(['clang',*flags,'-Wl,-dead_strip',str(root/'scripts/PureServerOracle.c'),*map(str,objects),'-o',str(out/'probe')],check=True)
print(out/'probe')
