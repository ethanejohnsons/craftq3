...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  ...
  if(sscanf(line,"random %d",&randomValue)==1){continue;}
  ...
  if(!strncmp(line,"goal ",5)) {
   ...
   api->ea.EA_ResetInput(ms->client);if(fields==10){api->ea.EA_Move(ms->client,seed,seedSpeed);api->ea.EA_Action(ms->client,seedFlags);}api->ea.EA_SelectWeapon(ms->client,7);vec3_t heldView={1,2,3};api->ea.EA_View(ms->client,heldView);randomDraws=0;api->ai.BotMoveToGoal(&result,mh,&g,flags);
   ...
   bot_input_t input;api->ea.EA_GetInput(ms->client,ms->thinktime,&input);printf("EA %.9g %.9g %.9g %.9g %d %d %.9g %.9g %.9g\n",input.dir[0],input.dir[1],input.dir[2],input.speed,input.actionflags,input.weapon,input.viewangles[0],input.viewangles[1],input.viewangles[2]);printf("DRAWS %d\n",randomDraws);
