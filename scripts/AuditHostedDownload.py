"""Both original gameplay ABIs with an authored PK3 download, packet loss, pure resume and official-pack refusal."""
...
for profile in (['retail','modern'] if args.profile=='both' else [args.profile]):
 out=root/'.tools/hosted-download'/profile;out.mkdir(parents=True,exist_ok=True)
 with tempfile.TemporaryDirectory(prefix='games-',dir=out) as temporary:
  ...
  command=[str(Path(os.environ['JAVA_HOME'])/'bin/java'),'-cp',args.classpath,str(root/'scripts/AuditHostedDownload.java'),str(games)]
  with (out/'audit.log').open('w') as log:result=subprocess.run(command,cwd=root,stdout=log,stderr=subprocess.STDOUT,timeout=90)
  if result.returncode:raise RuntimeError('Hosted download audit failed: '+str(out/'audit.log'))
