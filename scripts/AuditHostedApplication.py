"""Both original Create Server menus and actual host/remote sessions, only on loopback."""
...
for profile in (['retail','modern'] if args.profile=='both' else [args.profile]):
    out=root/'.tools/hosted-application'/profile;out.mkdir(parents=True,exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='games-',dir=out) as temporary:
        ...
        command=[str(root/'gradlew'),':craftq3-fabric:auditRemoteApplication','-Pq3HostAudit=true','-Pq3RemoteAuditGames='+str(games),'-Pq3RemoteAuditOutput='+str(out),'--console=plain']
        ...
        if result.returncode:raise RuntimeError('Hosted audit failed: '+str(out/'application.log'))
        print(profile+' '+(out/'hosted-application-result.json').read_text().strip(),flush=True)
