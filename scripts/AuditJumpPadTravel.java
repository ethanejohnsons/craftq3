...
/** Original-map differential for grounded jump-pad approach; the isolated oracle is never bundled. */
class AuditJumpPadTravel {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2 || args.length > 5)
      throw new IllegalArgumentException(
          "AuditJumpPadTravel <PK3> <oracle> [queries/map] [map|all] [entry|finish]");
    boolean finishing = args.length == 5 && args[4].equals("finish");
    if (args.length == 5 && !finishing && !args[4].equals("entry"))
      throw new IllegalArgumentException("Unknown jump-pad travel mode");
    ...
    try (var zip = new ZipFile(Path.of(args[0]).toFile())) {
      for (var entry :
        ...
        var movement = new JumpPadMovement(new MovementObstruction(CLEAR,
            a -> a <= 0 || a >= map.areas().size() ? 0 : map.areaSettings().get(a).reachabilityCount()));
        var reaches =
        ...
        for (int i = 0; i < count; i++) {
          var selected = reaches.get(random.nextInt(reaches.size()));
          var r = new dev.bluevista.craftq3.assets.aas.AasMap.Reachability(selected.area(), selected.face(), selected.edge(), selected.start(), selected.end(), 18, selected.travelTime(), 0);
          var start = r.start();
          Vec3 origin =
              start.add(
                  new Vec3(
          ...
          var input =
              new MovementInit(
                  ...
                  0);
          traceCount = 0;
          int sourceArea = nav.pointArea(input.origin());
          expected.add(finishing ? movement.finish(input, flags, sourceArea, r) : movement.execute(input, flags, sourceArea, r));
          counts.add(traceCount);
          commands.append("padpresence ").append(presence).append('\n');
          commands.append(finishing ? "pad 1 " : "pad 0 ");
          ...
          commands.append(input.thinkTime()).append(' ').append(flags).append(' ')
              .append(r.area()).append(' ').append(r.edge()).append(' ').append(r.face()).append(' ')
              .append(sourceArea).append(" 0\n");
        }
    ...
    System.out.printf(
        "Jump-pad " + (finishing ? "finish" : "entry") + " maps=%d requests=%d exactDifferences=%d maxFloatError=%.9g seconds=%.3f%n",
        maps, total, mismatches, maxError, (System.nanoTime() - begin) / 1e9);
    if (mismatches != 0) throw new AssertionError("Jump-pad travel differences: " + mismatches);
  }
  ...
  private static List<String> oracle(String executable, String pk3, String map, String commands)
    ...
    try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
      ...
      return text.lines().filter(l -> l.startsWith("PAD ")).toList();
    } finally {
